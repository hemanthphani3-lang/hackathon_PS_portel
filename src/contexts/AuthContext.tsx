...
interface AuthContextType {
  ...
  login: (teamName: string, password: string) => Promise<{ success: boolean; message: string; isAdmin: boolean }>;
  logout: () => void;
}
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = async (teamName: string, password: string): Promise<{ success: boolean; message: string; isAdmin: boolean }> => {
    // Admin check
    if (teamName === 'admin@1234' && password === 'admin@1234') {
      setSession({ teamId: 'admin', teamName: 'Admin', isAdmin: true, selectedMissionId: null });
    ...
    if (error || !result?.success) {
      return { success: false, message: result?.message || error?.message || 'Login failed', isAdmin: false };
    }
